...
/// a point in the activation space that is always valid, and default arguments
/// collapse the orthogonal axes (activation context, server) so that the
/// correct call is also the shortest one.
///
/// ## Axes of activation
///
...
///
/// The class-identity and server axes are handled by overloading and defaulting;
/// arity and aggregation change the *shape* of the result, so they select
/// distinct entry points.
///
/// ## Entry points
///
/// ### One interface — ``CoCreateInstance(_:activation:server:as:)``
///
...
/// to the `CLSID` form. `activation` and `server` default from
/// ``COMActivationOptions/current``, so a bare call inherits the ambient
/// configuration.
///
...
///
/// ### Several interfaces — ``CoCreateInstanceEx(_:activation:server:requesting:_:)``
///
...
///   is identical in effect to ``CoCreateInstance(_:activation:server:as:)``.
///   The single-interface `Ex` path exists only as an internal funnel, used
///   when a server is specified.
///
/// ## Choosing an entry point
///
/// - One interface, local or remote → ``CoCreateInstance(_:activation:server:as:)``
/// - Several interfaces at once → ``CoCreateInstanceEx(_:activation:server:requesting:_:)``
