...
namespace {
static ProtocolConformance *deriveImplicitCOMConformance(NominalTypeDecl *NTD,
  ...
  // Don't synthesize if the class already explicitly conforms.
  if (KP == KnownProtocolKind::ISwiftObject) {
    ...
    if (!conformances.empty()) {
      ...
      if (auto *A = CD->getAttrs().getAttribute<COMAttr>())
        context.Diags.diagnose(A->getLocation(), diag::attr_com_iswiftobject_implied);
