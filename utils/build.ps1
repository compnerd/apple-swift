      ...
      Windows {
        if ($UseASM) {
          ...
          if ($DebugInfo) {
            ...
            Add-FlagsDefine $Defines CMAKE_ASM_FLAGS $(& $Assembler.DebugFlags $DebugFormat)
          }
        }
...
function Build-LLVM([Hashtable] $Platform) {
  Build-CMakeProject `
    ...
    -CXXCompiler $Compilers.Builder.CXX `
    -Defines @{
      LLVM_HOST_TRIPLE = $Platform.Triple;
    }
}
